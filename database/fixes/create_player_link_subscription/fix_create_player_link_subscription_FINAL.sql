...
-- Problema identificado: La tabla access_codes tiene AMBAS columnas:
--   - active (boolean, NOT NULL, default: true)
--   - is_active (boolean, nullable)
-- 
-- La función usa coalesce(is_active, active) que causa error de tipos.
-- SOLUCIÓN: Usar solo 'active' porque es NOT NULL y tiene default true.
...
begin
  ...
  -- CORRECCIÓN: Usar solo 'active' (boolean, NOT NULL, default: true)
  -- NO usar coalesce(is_active, active) porque causa error de tipos
  if p_code_text is not null and length(trim(p_code_text)) > 0 then
    select id, num_days, max_uses, usage_count,
           active  -- CORREGIDO: Solo 'active', sin coalesce
...
BEGIN
  ...
  IF v_prosrc ILIKE '%coalesce(is_active%' THEN
    RAISE NOTICE '❌ PROBLEMA: La función aún contiene coalesce(is_active)';
    RAISE NOTICE 'Revisa manualmente el código fuente';
  ELSE
