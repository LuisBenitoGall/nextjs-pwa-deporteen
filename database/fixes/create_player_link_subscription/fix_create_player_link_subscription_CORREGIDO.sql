...
-- PROBLEMA: En el INSERT de subscription_players, puede haber ambigüedad con las columnas
-- de la tabla de retorno (RETURNS TABLE). También en el ON CONFLICT.
-- SOLUCIÓN: Calificar explícitamente las columnas en el ON CONFLICT y asegurar
-- que todas las referencias usen variables locales claramente nombradas
...
begin
  ...
  -- CORRECCIÓN: Usar variables locales y calificar columnas en ON CONFLICT
  ...
  -- NOTA: Las columnas en ON CONFLICT deben coincidir con el constraint único de la tabla
  -- Si el error persiste, puede ser que el constraint tenga otro nombre o esté en otras columnas

  -- 5) Retornar valores usando variables locales
