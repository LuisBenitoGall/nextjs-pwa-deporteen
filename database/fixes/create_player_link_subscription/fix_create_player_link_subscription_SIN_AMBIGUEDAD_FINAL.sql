...
-- PROBLEMA: El ON CONFLICT (subscription_id, player_id) es ambiguo porque estos nombres
-- también existen como columnas de salida en RETURNS TABLE.
-- SOLUCIÓN: Usar ON CONFLICT ON CONSTRAINT con el nombre del constraint, o calificar
-- explícitamente usando un alias de tabla.
...
-- Crear función con solución definitiva para la ambigüedad
...
begin
  ...
  -- SOLUCIÓN: Evitar ambigüedad verificando primero si existe el registro
  -- en lugar de usar ON CONFLICT que puede ser ambiguo con RETURNS TABLE
  if not exists (
    select 1 
  ...
  -- CORRECCIÓN: En lugar de ON CONFLICT que causa ambigüedad, verificamos primero
  -- si existe el registro usando un SELECT con alias de tabla 'sp' que califica
  -- explícitamente las columnas, evitando la colisión con RETURNS TABLE
...
-- y el ON CONFLICT ahora está calificado con el alias de tabla para evitar ambigüedad
